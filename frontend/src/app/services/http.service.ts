...
import {ApiLatestTestsResult, ApiTestsResult} from '../definitions/api';
...
export class HttpService {
  ...
  getTests(): Observable<ApiTestsResult[]> {
    return this.http.get<ApiTestsResult[]>(`${url}/tests`).pipe(
  ...
  getPreset(id): Observable<Preset> {
  ...
  deletePreset(id: string): Observable<any> {
    return this.http.delete<any>(`${url}/preset/${id}`).pipe(
      catchError(HttpService.handleError)
    );
  }
  ...
  savePreset(preset: Preset): Observable<any> {
    ...
    return this.http.post(`${url}/preset`, preset, httpOptions).pipe(
      catchError(HttpService.handleError)
    );
  }
}
